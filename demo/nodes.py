...
class Edge(StateObj):
    ...
    def calculate_state(self):
        ...
        self._state = self._input_objs[0].state and self._selected
...
class ORNode(StateObj):
    ...
    def calculate_state(self):
        self._state = any(obj.state for obj in self._input_objs)
...
class ANDNode(StateObj):
    ...
    def calculate_state(self):
        self._state = all(obj.state for obj in self._input_objs)
...
class NOTNode(StateObj):
    ...
    def calculate_state(self):
        self._state = not self._input_objs[0].state


class SwitchNode(StateObj):
    """Switch node. State is true if  the input edge is true. False otherwise.
    ...
    def calculate_state(self):
        self._state = self._input_objs[0].state
    ...
    def select_switch_edges(self, edge_indicies: Dict[int, bool]):
        ...
        if edge_indicies.keys() <= self._switch_selection.keys():
            raise KeyError(
                f"Unknown edges: {edge_indicies.keys() - self._switch_selection.keys()}!"
                f"Valid edges are: {set(edge_indicies)}"
                )
...
class LeafNode(ANDNode):
    ...
    State is true if the input edge is true. False otherwise.
